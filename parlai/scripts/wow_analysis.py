...
def display_data(opt):
    ...
    for _ in range(world.num_examples()):
        ...
        if opt.get('verbose', False) or opt.get('display_add_fields', ''):
            ...
            act = world.get_acts()[0]
            print(act.keys())
            checked_sentence = act['checked_sentence']
            eval_sentence = act['eval_labels'][0]
            test_Text =  act['text']
            print(checked_sentence)
            print(eval_sentence)
            print(test_Text)
            fuzz_partial_ratio = fuzz.partial_ratio(checked_sentence, eval_sentence)
            fuzz_token_set_ratio = fuzz.token_set_ratio(checked_sentence, eval_sentence)
            
            print(fuzz_token_set_ratio)
            if fuzz_token_set_ratio > 80:
                n_count += 1
        else:
